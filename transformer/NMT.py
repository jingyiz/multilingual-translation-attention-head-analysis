...
class NMT(nn.Module):
 ...
 def __init__(self,nogate, isize,cnnnum,cnnnumdec, snwd, tnwd, num_layer, fhsize=None, dropout=0.0, attn_drop=0.0, global_emb=False, num_head=8, xseql=cache_len_default, ahsize=None, norm_output=True, bindDecoderEmb=False, forbidden_index=None):

  super(NMT, self).__init__()

  enc_layer, dec_layer = parse_double_value_tuple(num_layer)

  self.enc = Encoder(nogate,isize,cnnnum, snwd, enc_layer, fhsize=fhsize, dropout=dropout, attn_drop=attn_drop, num_head=num_head, xseql=xseql, ahsize=ahsize, norm_output=norm_output)

  emb_w = self.enc.wemb.weight if global_emb else None

  self.dec = Decoder(nogate,isize,cnnnumdec, tnwd, dec_layer, fhsize=fhsize, dropout=dropout, attn_drop=attn_drop, emb_w=emb_w, num_head=num_head, xseql=xseql, ahsize=ahsize, norm_output=norm_output, bindemb=bindDecoderEmb, forbidden_index=forbidden_index)
 ...
 def forward(self, inpute, inputo,jprint,istrain, mask=None):

  _mask = inpute.eq(0).unsqueeze(1) if mask is None else mask
  jmask= inputo.eq(0).unsqueeze(1) 
  jleft1,jloss1, outj1=self.enc(inpute,jprint,istrain,_mask)
  jleft2,jloss2, outj2=self.dec(jmask,outj1, inputo,jprint,istrain, _mask)
  return outj2, jloss1+jloss2,jleft1+jleft2
 ...
 def decode(self, inpute, beam_size=1, max_len=None, length_penalty=0.0):
  ...
  _max_len = inpute.size(1)*10 + max(64, inpute.size(1)*10 // 4) if max_len is None else max_len

  return self.dec.decode(self.enc(inpute,False,False, mask)[2], mask, beam_size, _max_len, length_penalty)
