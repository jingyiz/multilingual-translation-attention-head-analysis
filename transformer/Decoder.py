...
class jdeccnn(nn.Module):
        
        
  def __init__(self ):
    super(jdeccnn, self).__init__()
    ...
    self.jact=nn.ReLU()
    self.cnn = nn.Conv1d(512,512,7,padding=6)
  def forward(self, inputs, mask=None):
    _inputs = self.layer_normer(inputs)
    if mask is not None:_inputs= _inputs.masked_fill_( mask.permute(0,2,1),0)
    context = self.jact(self.cnn(_inputs.permute(0,2,1)))+0
    context=context.permute(0,2,1)          
    context = self.drop(context)
    context = context[:,:-6,:] +    inputs
    return context                      
...
class DecoderLayer(nn.Module):
 ...
 def __init__(self,nogate, isize,cnnnum, fhsize=None, dropout=0.0, attn_drop=0.0, num_head=8, ahsize=None, norm_residual=norm_residual_default, k_rel_pos=use_k_relative_position_decoder):
  ...
  self.self_attn = SelfAttn(nogate,isize, _ahsize, isize, num_head=num_head, dropout=attn_drop, k_rel_pos=k_rel_pos, uni_direction_reduction=True)
  self.cross_attn = CrossAttn(nogate,isize, _ahsize, isize, num_head=num_head, dropout=attn_drop)
  ...
  if self.cnnnum>0:
    jtmp=[]
    for jjj in range(self.cnnnum):jtmp.append(jdeccnn())
 ...
 def forward(self,jmask, inpute, preinputo,jprint,istrain, src_pad_mask=None, tgt_pad_mask=None, prequery_unit=None):
  ...
  query_unit=None
  if self.cnnnum>0:
   if prequery_unit is None:
    inputo=preinputo+0
    for jjjjj in range(self.cnnnum):
      inputo=self.cnns[jjjjj](inputo,jmask)
   else:
    prek,prev,prejip=preinputo
    if prejip is None:currentjip=prequery_unit
    else:currentjip=torch.cat((prejip,prequery_unit),dim=1)
    inputo=(prek,prev,currentjip+0)
    for jjjjj in range(self.cnnnum):
      currentjip=self.cnns[jjjjj](currentjip,None)
    query_unit=currentjip[:,-1:,:]
  else:
    inputo=preinputo
    query_unit=prequery_unit
  if query_unit is None:
   _inputo = self.layer_normer1(inputo)

   context,jloss1,jleft1 = self.self_attn(_inputo,jprint, istrain,mask=tgt_pad_mask)
   ...
   context, states_return = self.self_attn(_query_unit, jprint,istrain,states=inputo)
  ...
  _context_new,jloss2,jleft2 = self.cross_attn(_context, inpute,jprint,istrain, mask=src_pad_mask)
  ...
  if query_unit is None:
   return context, jloss1+jloss2,jleft1+jleft2
  else:
...
class Decoder(nn.Module):
 ...
 def __init__(self,nogate, isize,cnnnum, nwd, num_layer, fhsize=None, dropout=0.0, attn_drop=0.0, emb_w=None, num_head=8, xseql=cache_len_default, ahsize=None, norm_output=True, bindemb=True, forbidden_index=None, share_layer=False, disable_pemb=disable_std_pemb_decoder):
  ...
  if share_layer:
   ...
   self.nets = nn.ModuleList([DecoderLayer(nogate,isize,cnnnum[i], _fhsize, dropout, attn_drop, num_head, _ahsize) for i in range(num_layer)])
 ...
 def forward(self, jmask,inpute, inputo, jprint,istrain,src_pad_mask=None):
  ...
  for net in self.nets:
   out,jloss ,jleft = net(jmask,inpute, out,jprint, istrain,src_pad_mask, _mask)
   if allloss is None:
       allloss=jloss
       allleft=jleft
   else:
 ...
 def beam_decode(self, inpute, src_pad_mask=None, beam_size=8, max_len=512, length_penalty=0.0, return_all=False, clip_beam=clip_beam_with_lp, fill_pad=False):
  ...
  for _tmp, net in enumerate(self.nets):
   out, _state = net(None,inpute, (None, None,None),False,False, src_pad_mask, None, out)
   states[_tmp] = _state
  ...
  for step in range(1, max_len):
   ...
   for _tmp, net in enumerate(self.nets):
    out, _state = net(None,inpute, states[_tmp],False,False, _src_pad_mask, None, out)
    states[_tmp] = _state
