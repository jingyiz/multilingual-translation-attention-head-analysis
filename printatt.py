...
def train(jweight,jprint,td, tl, ed, nd, optm, lrsch, model, lossf, mv_device, logger, done_tokens, multi_gpu, multi_gpu_optimizer, tokens_optm=32768, nreport=None, save_every=None, chkpf=None, chkpof=None, statesf=None, num_checkpoint=1, cur_checkid=0, report_eva=True, remain_steps=None, save_loss=False, save_checkp_epoch=False, scaler=None):
 ...
 for i_d in tqdm(tl):
  ...
  with autocast(enabled=_use_amp):
   output,jloss,_ = model(seq_batch, oi,jprint,True)
   loss = lossf(output, ot)
   if multi_gpu:
    loss = loss.sum()
   loss=loss+jweight*jloss.sum()
...
def eva(jprint,ed, nd, model, lossf, mv_device, multi_gpu, use_amp=False):
 r = w = 0
 sum_loss = 0.0
 model.eval()
 ...
 with torch.no_grad():
  for i in tqdm(range(nd)):
   ...
   with autocast(enabled=use_amp):
    output,jloss,jleft = model(seq_batch, seq_o.narrow(1, 0, lo),jprint,False)
...
lrsch = LRScheduler(optimizer, cnfg.isize, cnfg.warm_step, scale=cnfg.lr_scale,jstart=cnfg.jstart)
#lrsch.step()
...
minloss, minerr = eva(cnfg.jprint,vd, nvalid, mymodel, lossf, cuda_device, multi_gpu, use_amp)
...
if fine_tune_m is None:
 ...
 if (cnt_states is not None) and p_check(cnt_states):
  ...
  vloss, vprec = eva(cnfg.jprint,vd, nvalid, mymodel, lossf, cuda_device, multi_gpu, use_amp)
...
for i in range(1, maxrun + 1):
 shuffle(tl)
 free_cache(use_cuda)
 terr, done_tokens, cur_checkid, remain_steps, _Dws = train(cnfg.jweight,cnfg.jprint,td, tl, vd, nvalid, optimizer, lrsch, mymodel, lossf, cuda_device, logger, done_tokens, multi_gpu, multi_gpu_optimizer, tokens_optm, batch_report, save_every, chkpf, chkpof, statesf, num_checkpoint, cur_checkid, report_eva, remain_steps, dss_ws > 0, i >= start_chkp_save, scaler)
 vloss, vprec = eva(cnfg.jprint,vd, nvalid, mymodel, lossf, cuda_device, multi_gpu, use_amp)
