...
class jgate(nn.Module):
  def __init__(self,  nogate,temperature=0.33, stretch_limits=(-0.1, 1.1),
                               l0_penalty=1.0, l2_penalty=0.0, eps=1e-6, hard=False, local_rep=False):
    super(jgate, self).__init__()                  
    ...
    self.log_a =  nn.Embedding(8, 1)
    self.nogate=nogate
                                                                      
  def __call__(self, values, is_train ):
        
    gates = self.get_gates(is_train, self.log_a(values))

    if self.l0_penalty != 0 or self.l2_penalty != 0:
      reg = self.get_penalty(values=self.log_a(values), )
    if self.nogate:gates=gates*0+1
    return gates, reg,self.get_gates(False,self.log_a(values))

                                                                    
  def get_gates(self, is_train,jallg ):
            
    low, high = self.stretch_limits
                            
    if is_train:
       
      noise = torch.from_numpy(np.random.uniform(size=(8,1), low=self.eps,high= 1.0 - self.eps)).to(jallg.device).float()
      concrete = torch.sigmoid((torch.log(noise) - torch.log(1 - noise) + jallg) / self.temperature)
    else:
      concrete = torch.sigmoid(jallg)
    ...
    jminus=hard_concrete-clipped_concrete
    clipped_concrete = clipped_concrete + jminus.detach()
                                            

    
    return clipped_concrete.unsqueeze(0).unsqueeze(3)

                                                                                                                                                                                                        
  def get_penalty(self, values ):
    low, high = self.stretch_limits
    p_open = torch.sigmoid(values - self.temperature * math.log(-low / high))
...
class SelfAttn(nn.Module):
 ...
 def forward(self, iQ, jprint,istrain,mask=None, states=None):
  ...
  if states is not None:
   _h_real_iK, _h_real_iV,jipkeep = states
  ...
  if self.drop is not None:
   scores = self.drop(scores)
  jgates, jloss,jleft= self.jgatep(torch.from_numpy(np.arange(8)).to(scores.device),istrain)
  gatedout=scores.matmul(real_iV) * jgates
  if jprint:
         print(scores.cpu().detach().numpy().tolist())
         print(jgates.cpu().detach().numpy().tolist())
  out = self.outer(gatedout.transpose(1, 2).contiguous().view(bsize, nquery, self.hsize))

  if states is None:
   return out,jloss,jleft
  else:
   return out, (real_iK, real_iV,jipkeep)
...
class CrossAttn(nn.Module):
 ...
 def __init__(self,nogate, isize, hsize, osize, num_head=8, dropout=0.0, k_isize=None, enable_bias=enable_prev_ln_bias_default, enable_proj_bias=enable_proj_bias_default, sparsenorm=False):
 ...
 def forward(self, iQ, iK, jprint,istrain,mask=None):
  ...
  if jprint:
         print(scores.cpu().detach().numpy().tolist())
         print(jgates.cpu().detach().numpy().tolist())
  return self.outer(gatedout.transpose(1, 2).contiguous().view(bsize, nquery, self.hsize)),jloss,jleft
